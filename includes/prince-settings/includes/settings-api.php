...
if ( ! class_exists( 'Prince_Settings' ) ) {
	...
	class Prince_Settings {
		...
		public function display_setting( $args = array() ) {
			...
			// Set standard value.
			if ( isset( $std ) ) {
				$field_value = prince_filter_std_value( $field_value, $std );
			}

			// Allow the descriptions to be filtered before being displayed.
			$desc = apply_filters( 'prince_filter_description', ( isset( $desc ) ? $desc : '' ), $id );
			...
			if ( 'google-fonts' === $type ) {
				...
				prince_set_google_fonts( $id, $field_value );
			}

			// Get the option HTML.
			prince_display_by_type( $_args );
		}
				...
				foreach ( (array) $this->get_pages( $option ) as $page ) {
					...
					foreach ( (array) $this->get_the_settings( $page ) as $setting ) {
						...
						if ( isset( $setting['std'] ) ) {
							...
							$defaults[ $setting['id'] ] = prince_validate_setting( $setting['std'], $setting['type'], $setting['id'] );
						}
					}
				}
				...
				foreach ( (array) $this->get_pages( $option ) as $page ) {
					...
					foreach ( (array) $this->get_the_settings( $page ) as $setting ) {
						...
						if ( isset( $setting['type'] ) && isset( $input[ $setting['id'] ] ) ) {
							...
							// Get the defaults.
							$current_settings = get_option( prince_settings_id() );
							...
							if ( is_array( $input[ $setting['id'] ] ) && in_array( $setting['type'], array(
								...
								$settings = isset( $post_global[ $setting['id'] . '_settings_array' ] ) ? prince_decode( $post_global[ $setting['id'] . '_settings_array' ] ) : array();

								// Settings are empty for some odd reason get the defaults.
								if ( empty( $settings ) ) {
									$settings = 'slider' === $setting['type'] ? prince_slider_settings( $setting['id'] ) : prince_list_item_settings( $setting['id'] );
								}
								...
								if ( empty( $settings ) ) {
									$settings = prince_social_links_settings( $setting['id'] );
								}
								...
								foreach ( $input[ $setting['id'] ] as $k => $setting_array ) {
									...
									foreach ( $settings as $sub_setting ) {
										...
										if ( isset( $sub_setting['type'] ) && isset( $input[ $setting['id'] ][ $k ][ $sub_setting['id'] ] ) ) {
											...
											// Validate setting.
											$input_safe = prince_validate_setting( $input[ $setting['id'] ][ $k ][ $sub_setting['id'] ], $sub_setting['type'], $sub_setting['id'], $wpml_id );
								...
								$input[ $setting['id'] ] = prince_validate_setting( $input[ $setting['id'] ], $setting['type'], $setting['id'], $setting['id'] );
							}
						}
